...
@interface SHChineseSortModel : NSObject
//用将需要排序的对象封装在SHChineseSort对象中，包含排序的字符串，对象，首字母三个属性
//进行比较的字符串，
...
@implementation SHChineseSortModel
@end
...
+(void)sortAndGroup:(NSArray*)objectArray
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [objectArray enumerateObjectsWithOptions:NSEnumerationConcurrent usingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            SHChineseSortModel *model = [SHChineseSort getModelWithObj:obj key:key];
            if (model) {
                //对 数组的插入操作 上锁
        ...
        NSSortDescriptor *sortDescriptor = [NSSortDescriptor sortDescriptorWithKey:@"pinYin" ascending:YES selector:@selector(sort:)];
        ...
        for (SHChineseSortModel* object in sortModelArray) {
...
+ (SHChineseSortModel*)getModelWithObj:(id)obj key:(NSString*)key{
    SHChineseSortModel *model = [[SHChineseSortModel alloc]init];
    model.object = obj;
    if (!key) {
        model.string = obj;
    }else{
...
#pragma mark ===============获取汉字首字母====================
//获得 首字母组成的字符串保留非中文字符  成都->CD  abc->ABC sc成都->SCCD
